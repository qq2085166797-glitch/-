...
3. Produce the pre-generation direction package: copy direction, style direction, and visual content direction.
...
For IP products, do not invent unofficial characters, logos, slogans, licensing text, co-branded claims, or brand partnerships. Use official IP graphics only when the user supplies them or they are visible on the provided product/package. Otherwise, borrow only the IP-adjacent mood such as palette, scene warmth, playful density, girl-cute styling, collectible styling, or giftable styling.
...
- PINGU products can use icy blue, white, clean snow-like materials, cool cute cards, and clear product proof for student, gift, or daily-use buyers.
...
Before the full blueprint, present a compact direction package and stop for confirmation. It must include:
...
- **Style direction**: IP identity, product category tone, target audience, style-fit reason, palette, analogous color extension, small contrast-color accents, lighting, typography direction, text layout reference, card/icon language, prop density, layout diversity plan, and commercial-font/IP-text safety rule.
- **Visual content direction**: planned hero product treatment, required scenes, model-use scene, backpack portability scene, packaging/list treatment, props, product angles, and what must not be hidden.
...
Do not write the screen-by-screen blueprint until the user confirms this direction package. If the user revises the direction, update the direction package first, then continue to the blueprint.
...
When the user supplies detail-page references for text layout, borrow their typography arrangement and information rhythm: large playful headline blocks, short subtitle lines, sticker labels, small badge text, arrow callouts, speech bubbles, bracket labels, rounded information cards, mini comparison boxes, parameter sheets, and tilted photo/card captions. Keep text short and readable. Do not copy the reference's exact visible text, commercial font, protected logo lettering, official IP wordmark, brand slogan, character title treatment, or unrelated product claims.
...
- IP tone, scene choice, and typography fit the target audience without adding unauthorized IP elements or commercial fonts.
...
- **Text master**: exact visible Chinese copy, text layout reference, copy hierarchy, claim-source notes, parameter placeholders, and prohibited wording.
- **Visual master**: color system, analogous color extension, contrast accent limits, lighting direction, background material, product angle rules, typography, text layout system, icon style, card radius, shadow, margins, prop level, SKU rules, continuity motifs, layout diversity plan, and per-screen pacing.
...
Then add the screen-specific composition, copy, text layout reference, hierarchy, layout archetype, layout variation reason, edge anchors, and style notes from the approved blueprint.

When an IP style is involved, add the approved IP style direction after the product-faithfulness base. Keep the product and supplied artwork accurate; use IP mood to guide scene, palette, props, and layout. The scene and layout palette may add neighboring analogous colors and a small amount of contrast-color accents for stickers, arrows, badges, icons, or small decoration, but the product's real SKU colors and official artwork colors must remain unchanged. For Sanrio, Crayon Shin-chan, PINGU, Disney, tokidoki, Brunch Brothers, and similar IP products, girl-cute, cute, giftable, and collectible styling is allowed when it fits the target audience. Small glossy PVC-style figure props may be used as secondary accents, but do not add unofficial characters, unsupplied logos, official slogans, commercial-font names, or props that imply unconfirmed bundle contents.
...
- **IP-style images**: adapt the scene and layout to the product's IP tone and target audience. Girl-cute/cute styling and small PVC-style figure accents are allowed for suitable IP products, but only use official artwork supplied by the user or visible on the product/package.
...
Fail a result if it contains product redesign, wrong proportions, wrong port/button/screen/line/hinge position, missing real structure, extra parts, wrong SKU color, wrong package shape, invented parameters, unauthorized IP elements, PVC-style figures that dominate or imply unconfirmed accessories, commercial-font names, unreadable critical text, copied reference text, protected wordmark imitation, contrast colors that overpower the product, palette shifts that change the product/IP identity, repeated standalone-poster layouts, repeated adjacent layout grammar, insufficient layout diversity, or broken page continuity.
